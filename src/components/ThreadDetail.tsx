...
export default function ThreadDetail({
  ...
  // Always call hooks - handle null thread in the render logic
  ...
  // Initialize editedBody when draft changes
  useEffect(() => {
    if (draft && draft.body) {
  ...
  }, [thread, messages, onSummaryGenerated, onNotify, onCreditsUsed]);

  useEffect(() => {
    if (isCollapsed && !summary && !summaryLoading && thread) {
      handleGenerateSummary();
    }
  }, [isCollapsed, summary, summaryLoading, thread?.id, handleGenerateSummary]);
  ...
  const handleAcceptImprovement = () => {
    ...
    onUpdateDraft(draft.id, streamingReply);
  ...
  // Handle null thread case after all hooks have been called
  if (!thread) {
    return (
    ...
    <div className="flex-1 flex">
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        ...
        <Card className="mb-6 dark:bg-gray-800 dark:border-gray-700">
          <CardHeader>
            <CardTitle className="text-2xl text-gray-900 dark:text-white">{thread.subject}</CardTitle>
        ...
        {/* Collapsed Summary */}
        {isCollapsed && (
          <Card className="border-2 border-blue-200 bg-blue-50 dark:border-blue-700 dark:bg-blue-900/20 dark:bg-gray-800">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300">Summary</Badge>
              </div>
            </CardHeader>
            <CardContent>
              {summaryLoading ? (
                <div className="flex items-center gap-2">
                  ...
                  <span className="italic text-sm text-gray-300 dark:text-gray-300">Generating summary...</span>
                </div>
              ) : (
                <p className="text-gray-200 dark:text-gray-200 whitespace-pre-wrap mb-4">{summary || 'No summary available.'}</p>
              )}
            </CardContent>
          </Card>
        )}
        ...
        <div className="space-y-4 mb-6">
          {messages.map((message, idx) => (
            <Card key={message.id} className="relative dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-semibold text-gray-900 dark:text-white">{message.sender}</span>
                      <span className="text-xs text-gray-300 dark:text-gray-400">
                        {new Date(message.date).toLocaleString()}
                      </span>
                    </div>
                    <p className="text-gray-200 dark:text-gray-200 whitespace-pre-wrap">{message.body}</p>
                  </div>
                </div>
              </CardContent>
              {idx < messages.length - 1 && (
                <div className="absolute left-7 top-14 bottom-0 w-0.5 bg-gray-200 dark:bg-gray-600" />
              )}
            </Card>
          ))}
        </div>
        ...
        {!isCollapsed && draft && (
          <Card className={`border-2 ${draft.status === 'sent' ? 'border-green-200 bg-green-50 dark:border-green-700 dark:bg-green-900/20' : draft.status === 'rejected' ? 'border-red-200 bg-red-50 dark:border-red-700 dark:bg-red-900/20' : 'border-blue-200 bg-blue-50 dark:border-blue-700 dark:bg-blue-900/20'} dark:bg-gray-800`}>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg flex items-center gap-2 text-gray-900 dark:text-white">
                  <span className={draft.status === 'sent' ? 'text-green-600 dark:text-green-400' : draft.status === 'rejected' ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400'}>
                ...
                <Badge className={
                  draft.status === 'sent' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 
                  draft.status === 'rejected' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' : 
            ...
            <CardContent>
              <Textarea
                ...
                className="resize-none min-h-[200px] w-full text-base leading-relaxed focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white mb-4 text-gray-900"
              ...
              {showImproveChat && (
                <div className="mb-4 border-t dark:border-gray-600 pt-3 space-y-3">
                  <div className="flex gap-2">
                    <Textarea
                      ...
                      className="flex-1 resize-none text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white"
      ...
      {/* Knowledge Sidebar */}
      <aside className="w-80 border-l bg-gray-50 dark:bg-gray-800 dark:border-gray-700 p-4 overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-lg text-gray-900 dark:text-white">Knowledge Base</h3>
          <Link href="/knowledge" className="text-xs text-blue-600 dark:text-blue-400 hover:underline">See more</Link>
        </div>
        
        {/* People */}
        <div className="mb-6">
          <h4 className="font-semibold mb-2 text-gray-200 dark:text-gray-200">People</h4>
          <div className="space-y-2">
            {relatedPeople.length === 0 && <p className="text-xs text-gray-300 dark:text-gray-400">No people found.</p>}
            {relatedPeople.map((person) => (
              <Card key={person.id} className="p-3 cursor-pointer dark:bg-gray-700 dark:border-gray-600" onClick={() => setActivePerson(person)}>
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-300 dark:text-gray-400 truncate">{person.email}</p>
                    {person.company && (
        ...
        {/* Projects */}
        <div>
          <h4 className="font-semibold mb-2 text-gray-200 dark:text-gray-200">Projects</h4>
          <div className="space-y-2">
            {relatedProjects.length === 0 && <p className="text-xs text-gray-300 dark:text-gray-400">No projects found.</p>}
            {relatedProjects.map((project) => (
              <Card key={project.id} className="p-3 dark:bg-gray-700 dark:border-gray-600">
                <h5 className="font-medium text-sm text-gray-900 dark:text-white">{project.name}</h5>
                <p className="text-xs text-gray-200 dark:text-gray-300 mt-1">{project.description}</p>
                <Badge className="mt-2 text-xs dark:bg-gray-600 dark:text-gray-200 dark:border-gray-500" variant="outline">
                  {project.status}
                </Badge>
              </Card>
            ))}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setShowModal(false)}>
          <div className="bg-white dark:bg-gray-800 rounded shadow p-6 w-96 max-h-[80vh] overflow-y-auto" onClick={(e)=>e.stopPropagation()}>
            <h3 className="font-bold text-lg mb-4 text-gray-900 dark:text-white">All People</h3>
            <div className="space-y-2">
              {people.map((person) => (
                <Card key={person.id} className="p-3 dark:bg-gray-700 dark:border-gray-600">
                  <p className="font-medium text-sm text-gray-900 dark:text-white">{person.name}</p>
                  <p className="text-xs text-gray-300 dark:text-gray-400">{person.email}</p>
                </Card>
              ))}
            </div>
            <h3 className="font-bold text-lg mt-6 mb-4 text-gray-900 dark:text-white">All Projects</h3>
            <div className="space-y-2">
              {projects.map((project) => (
                <Card key={project.id} className="p-3 dark:bg-gray-700 dark:border-gray-600">
                  <p className="font-medium text-sm text-gray-900 dark:text-white">{project.name}</p>
                  <p className="text-xs text-gray-200 dark:text-gray-300">{project.description}</p>
                </Card>
              ))}
            </div>
      ...
      {activePerson && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={()=>setActivePerson(null)}>
          <div className="bg-white dark:bg-gray-800 rounded shadow p-6 w-80" onClick={(e)=>e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-300 dark:text-gray-400">{activePerson.email}</p>
              </div>
            </div>
            {activePerson.company && (
              <p className="text-sm mb-2 text-gray-900 dark:text-white"><strong>Company:</strong> <span className="text-gray-200 dark:text-gray-300">{activePerson.company}</span></p>
            )}
            {activePerson.role && (
              <p className="text-sm mb-2 text-gray-900 dark:text-white"><strong>Role:</strong> <span className="text-gray-200 dark:text-gray-300">{activePerson.role}</span></p>
            )}
            {activePerson.notes && (
              <p className="text-sm mb-2 text-gray-900 dark:text-white"><strong>Notes:</strong> <span className="text-gray-200 dark:text-gray-300">{activePerson.notes}</span></p>
            )}
            <div className="text-right mt-4">
